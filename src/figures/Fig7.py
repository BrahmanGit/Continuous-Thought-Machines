...
- Left: ECDF (Empirical Cumulative Distribution Function) of negative latency
- Right: Stacked bar chart showing object class distribution across quantile ranges

This visualization shows how early predictions occur and which objects are 
predicted at different stages of the movement.
"""
import sys
...
# Filter to all-fingers condition only
finger_df = all_df[
...
for _, row in finger_df.iterrows():
    if row['correct'] == 1:
        ...
        nl2 = max(row['t_nl2'] - row[time], row['t_hand'] - row[time])
        nl3 = max(row['t_nl3'] - row[time], row['t_hand'] - row[time])
...
# Print percentage of rows with negative 'nl' for each k value
for i in range(1, 4):
    k_df = plot_df[plot_df['type'] == f'k={i}']
    percent_negative = (k_df['nl'] < 0).sum() / len(k_df) * 100
    print(f"{percent_negative:.2f}% of k={i} rows have nl < 0")
...
legend1 = axes[0].legend(handles=target_handles, title="", loc='upper left',
                        fontsize=font_size, title_fontsize=font_size, ncol=1, frameon=True)
axes[0].add_artist(legend1)
legend1.get_frame().set_alpha(None)
axes[0].set_ylabel("Trials anticipated", fontsize=font_size)
...
axes[1].set_yticklabels(targets, fontsize=font_size)

ax2 = axes[1].twinx()
yticks_right = group_positions
yticklabels_right = [k[-1] for _ in targets for k in ks[::-1]]
ax2.set_yticks(group_positions)
ax2.set_yticklabels(yticklabels_right, fontsize=font_size)
ax2.set_ylabel("k value", fontsize=font_size)
ax2.yaxis.set_ticks_position('right')
ax2.yaxis.set_label_position('right')
ax2.set_ylim(axes[1].get_ylim())

for y in yticks_right:
    axes[1].axhline(y=y, color='lightgrey', linewidth=1, zorder=-10, linestyle='-')
...
axes[0].set_xlim([-1900, 1000])
axes[1].set_xlabel('Latency [ms]', fontsize=font_size)
axes[1].grid(False)
axes[0].grid(False)
...
plt.tight_layout()
plt.savefig('Fig7_rev.pdf', bbox_inches='tight')
